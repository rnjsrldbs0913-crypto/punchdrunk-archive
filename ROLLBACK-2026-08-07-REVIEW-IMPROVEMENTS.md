...
- 6번, 신청 후보 담기와 모아보기: `requestShortlist`
- 7번, 목록 커버에서 상세 커버로 이어지는 전환 효과: `coverTransitions`
- 8번, 작은 안내 문구와 보조 정보의 명암 강화: `higherContrast`

예를 들어 신청 후보 기능만 빼려면 다음처럼 바꿉니다.

```js
requestShortlist: false,
```
...
- 신청 후보는 손님의 현재 브라우저에만 저장됩니다. 서버나 관리자 데이터에는 아무 내용도 추가되지 않습니다.
- 전환 효과는 브라우저 지원 여부에 따라 자연스러운 커버 이동 또는 짧은 대체 효과로 보입니다.
